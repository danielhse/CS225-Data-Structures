...
class KDTree
{
  ...
  public:
    ...
    void findtheNearestNeighbor(KDTreeNode* curr_node, Point<Dim> &currentBest,
  ...
  private:
     ...
     */
    void helperKD(vector<Point<Dim>>& newPoints, int left, int right,
                    int dimension, KDTreeNode*& subroot);
    int part(vector<Point<Dim>>& list, int left, int right, size_t Idx, int dimension);
    Point<Dim> select(vector<Point<Dim>>& list, int left, int right, size_t k, int dimension);
    void copy_(KDTreeNode*& curr_node, KDTreeNode*& other);
