...
{
    /* Your code goes here! */
    string newword11= word1;
    string newword22= word2;
    std::transform(newword11.begin(), newword11.end(), newword11.begin(), ::toupper);
    std::transform(newword22.begin(), newword22.end(), newword22.begin(), ::toupper);
    if (dict.count(newword11) != true || dict.count(newword22) != true) 
        return 0;
    std::vector<std::string> first = dict.at(newword11);
    std::vector<std::string> second = dict.at(newword22);
    if (first != second) 
        return 0;
    else 
        return 1;
